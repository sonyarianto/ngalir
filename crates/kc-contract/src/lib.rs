//! Shared Node Contract for Kucur.
//!
//! Every `kc-*` node implements this uniform interface:
...
/// Capability manifest emitted by `kc-* --describe`.
...
pub struct Manifest {
    ...
    #[serde(default = "Value::default")]
    pub outputs: Value,
    /// Names of input fields that are credentials (resolved via `kc-vault`).
    #[serde(default)]
...
mod tests {
    ...
    fn test_manifest_defaults() {
        let m: Manifest =
            serde_json::from_str(r#"{"name":"kc-x","version":"0.1.0","description":"d"}"#).unwrap();
