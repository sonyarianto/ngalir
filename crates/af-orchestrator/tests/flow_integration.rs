...
fn axisflow_bin() -> PathBuf {
    let mut p = target_dir();
    p.push(format!("axisflow{}", std::env::consts::EXE_SUFFIX));
    assert!(p.exists(), "axisflow binary not found at {}", p.display());
    p
}
...
fn echo_demo_flow_succeeds() {
    let output = Command::new(axisflow_bin())
        ...
        .env("PATH", node_path())
        .output()
        .expect("axisflow process should start");
