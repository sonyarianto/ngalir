//! Kucur credential storage.
//!
//! Reads secrets from a JSON vault file (default `~/.kucur/vault.json`
//! or `KUCUR_VAULT_FILE`). Resolves `vault://<key>` references.

use kc_contract::{exit_code, fail, print_manifest, read_input, Manifest};
use serde_json::Value;
use std::path::PathBuf;

fn manifest() -> Manifest {
    Manifest {
        name: "kc-vault".to_string(),
...
fn vault_path() -> PathBuf {
    if let Ok(p) = std::env::var("KUCUR_VAULT_FILE") {
        return PathBuf::from(p);
    }
    let home = dirs_fallback();
    home.join(".kucur").join("vault.json")
}
...
fn load_vault() -> serde_json::Map<String, Value> {
    ...
    match std::fs::read_to_string(&path) {
        Ok(raw) => match serde_json::from_str::<Value>(&raw) {
            ...
            Ok(_) => {
                eprintln!("kc-vault: {} is not a JSON object", path.display());
                Default::default()
            }
            Err(e) => {
                eprintln!("kc-vault: failed to parse {}: {e}", path.display());
                Default::default()
            }
        },
        Err(e) => {
            eprintln!(
                "kc-vault: cannot read {} ({}): {}",
                path.display(),
                e.kind(),
                e
            );
