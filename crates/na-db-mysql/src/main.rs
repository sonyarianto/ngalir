...
/// Return the capability manifest for `na-db-mysql`.
fn manifest() -> Manifest {
    Manifest {
        name: "na-db-mysql".to_string(),
        version: env!("CARGO_PKG_VERSION").to_string(),
        description: "Execute SQL queries against MySQL databases.".to_string(),
        inputs: serde_json::json!({
            "type": "object",
            "properties": {
                "connection": { "type": "string", "description": "MySQL DSN or vault:// ref" },
        ...
        examples: vec![],
        see_also: vec!["db-postgres".into(), "db-sqlite".into()],
    }
}
...
async fn run() {
    ...
    if is_select {
        let rows: Vec<MySqlRow> = sqlx::query::<sqlx::MySql>(query)
            .fetch_all(&pool)
            .await
        ...
        let outcome = sqlx::query::<sqlx::MySql>(query)
            .execute(&pool)
            .await
            .unwrap_or_else(|e| fail(exit_code::GENERIC, format!("query failed: {e}")));
...
/// Convert a column value from a MySQL row to a JSON value.
fn value_at(row: &MySqlRow, col: &str) -> Value {
