...
//! wiring, then executes nodes as `kc-<use>` subprocesses with bounded
//! concurrency. Inter-node data is piped as JSON via stdin/stdout.

use kc_contract::Manifest;
...
enum Commands {
    ...
    /// List all available kc-* node binaries on PATH / KUCUR_NODE_PATH
    Nodes,
...
async fn cmd_nodes() -> Result<()> {
    ...
    if binaries.is_empty() {
        println!("No kc-* node binaries found on PATH or KUCUR_NODE_PATH.");
        return Ok(());
    }
...
async fn preflight(flow: &FlowSpec) -> Result<HashMap<String, NodeBin>> {
    ...
    for node in &flow.nodes {
        ...
        for req in &requireds {
            ...
            if !provided {
                bail!(
                    "node '{}' (kc-{}) is missing required input '{}'",
                    node.id,
                    node.use_,
                    req
                );
        ...
        if schema.is_object() {
            jsonschema::validator_for(schema).map_err(|e| {
                anyhow::anyhow!("invalid JSON Schema in manifest for kc-{}: {e}", node.use_)
            })?;
        }
...
fn find_in_node_path(name: &str) -> Option<String> {
    let node_path = std::env::var("KUCUR_NODE_PATH").ok()?;
...
async fn describe_binary(path: &str) -> Result<NodeBin> {
    ...
    let child = cmd.spawn().with_context(|| {
        format!("cannot find node binary '{path}' — ensure it is on PATH or set KUCUR_NODE_PATH")
    })?;
...
fn scan_binaries() -> Vec<String> {
    ...
    if let Ok(p) = std::env::var("KUCUR_NODE_PATH") {
    ...
    for dir in dirs {
        ...
        for e in entries.flatten() {
            ...
            if name.starts_with("kc-") && is_executable(&e) {
                seen.insert(name);
            }
        }
    }
...
async fn execute_node(
    ...
    loop {
        ...
        if code == kc_contract::exit_code::RETRYABLE && attempt < max_retries {
            attempt += 1;
            warn!(
                attempt,
                max_retries,
...
async fn call_vault_resolve(ref_str: &str) -> Result<String> {
    let mut cmd = Command::new("kc-vault");
    cmd.stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped());

    let mut child = cmd.spawn().context("spawn kc-vault (is it on PATH?)")?;
    {
        let mut stdin = child.stdin.take().context("kc-vault stdin")?;
    ...
    if !out.status.success() {
        bail!(
            "kc-vault resolve failed: {}",
            String::from_utf8_lossy(&out.stderr).trim()
        );
    }
    let result: Value = serde_json::from_slice(&out.stdout).context("parse kc-vault output")?;
    Ok(result["secret"]
        .as_str()
        .context("kc-vault response missing 'secret' field")?
        .to_string())
}
