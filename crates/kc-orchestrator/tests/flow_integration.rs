...
fn kucur_bin() -> PathBuf {
    let mut p = target_dir();
    p.push(format!("kucur{}", std::env::consts::EXE_SUFFIX));
    assert!(p.exists(), "kucur binary not found at {}", p.display());
    p
}
...
fn echo_demo_flow_succeeds() {
    let output = Command::new(kucur_bin())
        .arg("run")
        .arg(demo_flow())
        .env("PATH", node_path())
        .output()
        .expect("kucur process should start");
